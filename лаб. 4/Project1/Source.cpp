...
int levenshteinDistanceDP(const string& str1, const string& str2, int len1, int len2) {
	...
	for (int i = 0; i <= len1; ++i) {
		for (int j = 0; j <= len2; ++j) {
			if (i == 0) { // Если одна из строк пустая
				dp[i][j] = j; // Расстояние равно длине другой строки
			}
			else if (j == 0) { // Если одна из строк пустая
				dp[i][j] = i; // Расстояние равно длине другой строки
			}
			else if (str1[i - 1] == str2[j - 1]) { // Если символы в текущих позициях совпадают
				dp[i][j] = dp[i - 1][j - 1]; // Берем значение из диагональной ячейки
			}
			else {
				// Иначе, вычисляем минимальное значение из трех возможных случаев: вставки, удаления и замены
...
int levenshteinDistanceRecursive(const string& str1, int len1, const string& str2, int len2) {
	// Базовые случаи
	if (len1 == 0) return len2; // Если первая строка пустая, расстояние равно длине второй строки
	if (len2 == 0) return len1; // Если вторая строка пустая, расстояние равно длине первой строки

	// Если последние символы совпадают
	if (str1[len1 - 1] == str2[len2 - 1]) {
		return levenshteinDistanceRecursive(str1, len1 - 1, str2, len2 - 1); // Рекурсивный вызов для оставшихся подстрок
	}

	// Иначе, вычисляем минимальное значение из трех возможных случаев и добавляем 1
	return 1 + min(
		levenshteinDistanceRecursive(str1, len1, str2, len2 - 1),   // Вставка: рекурсивный вызов для первой строки и уменьшенной второй строки
		levenshteinDistanceRecursive(str1, len1 - 1, str2, len2),   // Удаление: рекурсивный вызов для уменьшенной первой строки и второй строки
		levenshteinDistanceRecursive(str1, len1 - 1, str2, len2 - 1) // Замена: рекурсивный вызов для уменьшенных первой и второй строк
	);
}
